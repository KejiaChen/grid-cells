...
class RandomMaze(object):
    def __init__(self, level, observations, configs, coord_range, reward_type='sparse-0', dist_epsilon=60):
        """
        ...
        self._observation_names = observations + ['RGB.LOOK_RANDOM_VIEW', 'RGB.LOOK_TOP_DOWN_VIEW']
    ...
    def observe(self):
        ...
        if self._lab.is_running():  # If the maze is still running
            ...
            self._trans = self._trans.copy()
            self._rots = self._rots
            self._vel_trans = self._vel_trans
            self._vel_rots = self._vel_rots
            self._vel_ego = self._vel_ego.copy()
            # set the distance
    ...
    def step(self, action, action_repeat):
        ...
        if self._lab.is_running():  # If the maze is still running
            ...
            self._trans = self._trans = list(map(self.position_scale, self._current_state['DEBUG.POS.TRANS']))[:2]
            self._rots = math.radians(self._current_state['DEBUG.POS.ROT'][1])
            # print("raw rots", self._current_state['DEBUG.POS.ROT'][1])
            # self._trans = [pos_x, pos_y, pos_z]
            # self._rots = [pos_pitch, pos_yaw, pos_roll]
            # update the velocity
            self._vel_trans = np.sqrt(np.square(self.position_scale(self._current_state['VEL.TRANS'][0]))
    ...
    def compute_reward(self, dist):
        if self.reward_type == 'sparse-0':
            reward = 10 if dist < self.dist_epsilon else 0
