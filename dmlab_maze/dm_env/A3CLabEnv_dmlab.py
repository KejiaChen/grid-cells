'''
Set the evironment level. Wrap-up Apis based on methods of deepmindlab.Lab
Check details on: https://github.com/deepmind/lab/blob/master/docs/users/python_api.md
'''
...
# def position_scale(map_pos, coord_range):
#     # x_scale = 2.5/(883-116)
#     x_scale = coord_range/(1083 - 115)
#     # y_scale = 2.5 / (1083 - 116)
#     real_pos = -0.5*coord_range + (map_pos-116)*x_scale  # rescale position to (-1.25, 1.25)
#     return real_pos
...
class RandomMaze(object):
    def __init__(self, level, observations, configs, coord_range, reward_type='sparse-0', dist_epsilon=35):
        """
        ...
        # self._observation_names = observations + ['RGB.LOOK_RANDOM_VIEW', 'DEBUG.POS.TRANS', 'DEBUG.POS.ROT', 'VEL.TRANS', 'VEL.ROT']
        ...
        # orientation info
        self._rots = None
        # translation velocity info
        self._vel_trans = None  # in meters
        # rotational velocity info
        self._vel_rots = None  # in radians
        # egocentric velocity info:
        self._vel_ego = None  # array[vel_trans, sin(vel_rots), cos(vel_rots)]
    ...
    #     return self._lab.action_spec()

    def position_scale(self, raw_pos):
        # x_scale = 2.5/(883-116)
    ...
    def reset(self, configs):
        ...
        self._top_down_obs = self._current_state['DEBUG.CAMERA.TOP_DOWN']
    ...
    def observe(self):
        ...
        if self._lab.is_running():  # If the maze is still running
            # get the next state
            self._current_state = self._lab.observations()
            # print("angular velocity observation:", (self._lab.observations())['VEL.ROT'])
            ...
            # get the next translation velocity
            vel_x, vel_y, vel_z = self._current_state['VEL.TRANS'].tolist()
            # get the next angular velocity
            vel_pitch, vel_yaw, vel_roll = self._current_state['VEL.ROT'].tolist()
            ...
            self._trans = self._trans = list(map(self.position_scale, self._current_state['DEBUG.POS.TRANS']))[:2]
            self._rots = math.radians(self._current_state['DEBUG.POS.ROT'][1])
            ...
            self._vel_trans = np.copy(self._vel_trans)
            self._vel_rots = np.copy(self._vel_rots)
            self._vel_ego = np.copy(self._vel_ego)
            # set the distance
            self._last_distance = self._last_distance

        return self._last_observation, reward, terminal, self._last_distance, self._trans, self._rots, self._vel_ego, dict()
    ...
    def step(self, action, action_repeat):
        ...
        self._lab.step(action, num_steps=action_repeat)
    ...
    def show_front_view(self, time_step=None):
        assert len(self._last_observation.shape) == 3, f"Invalid observation, expected observation should be " \
                                                       f"RGB_INTERLEAVED, but get {self._observation_names[0]}." \
                                                       f" Please check the observation list. The first one should be" \
                                                       f" RGB_INTERLEAVED."
        # init or update data
        if time_step is None:
            ...
            self.img_artists.append(self.arrays[1].imshow(ndimage.rotate(self._top_down_obs.transpose(1, 2, 0), -90)))
        else:
            self.img_artists[0].set_data(self._last_observation)
            self.img_artists[1].set_data(ndimage.rotate(self._top_down_obs.transpose(1, 2, 0), -90))
        self.fig.canvas.draw()
        plt.pause(0.0001)
