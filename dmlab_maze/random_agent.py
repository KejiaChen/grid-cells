...
class SpringAgent(object):
    ...
    def __init__(self, action_spec):
        ...
        # to be changed to make speed 15cm/s = 0.6 grid/s
        self.velocity_scaling = np.array([2.5, 2.5, 0.01, 0.01, 0.01, 0.01])
    ...
    def critically_damped_derivative(self, t, omega, displacement, velocity):
        r"""Critical damping for movement. (Spring-like forces)
    ...
    def random_step(self):
        """Gets an image state and a reward, returns an action."""
        action_index = int(random.uniform(0, len(self.action_spec) - 1))
        action = self.action_spec[action_index]
        velocity_scaling = self.velocity_scaling[action_index]

        # A naive kinematic which assumes the robot have a constant velocity
        self.action = velocity_scaling * action
...
def run(length, width, height, fps, level, record, demo, demofiles, video):
    ...
    if video:
        config['video'] = video
    env = deepmind_lab.Lab(level, ['RGB_INTERLEAVED', 'DEBUG.POS.TRANS', 'DEBUG.POS.ROT'], config=config)    # choose observation here
    ...
    for _ in six.moves.range(length):
        ...
        print("position of robot:", obs['DEBUG.POS.TRANS'])
        action = agent.step(reward, obs['RGB_INTERLEAVED'])
        reward = env.step(action, num_steps=1)

    print('Finished after %i steps. Total reward received is %f' % (length, agent.rewards))
