...
def circle_mask(size, radius, in_val=1.0, out_val=0.0):
    ...
    vfunc = np.vectorize(lambda b: b and in_val or out_val)  # (z<=r && in || out)
    return vfunc(z)
...
class GridScorer(object):
    ...
    def _get_ring_mask(self, mask_min, mask_max):  # get a ring from min to max
    ...
    def grid_score_60(self, corr):  # ref20: gridness > 0, i.e. more correlations at 60 and 120 than 30, 90, 150
        if self._min_max:
    ...
    def get_grid_scores_for_mask(self, sac, rotated_sacs, mask):
        ...
        corrs = dict()
        for angle, rotated_sac in zip(self._corr_angles, rotated_sacs):  # calculate cov for different rotating angles
            masked_rotated_sac = (rotated_sac - masked_sac_mean) * mask
            cross_prod = np.sum(masked_sac_centered * masked_rotated_sac) / ring_area  # E[(X-E(X)(Y-E(Y))]
            corrs[angle] = cross_prod / variance  # covariance
