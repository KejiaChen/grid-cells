...
def one_hot_max(x, axis=-1):
    ...
    return tf.one_hot(tf.argmax(input=x, axis=axis),
                      depth=x.get_shape()[-1],
                      dtype=x.dtype)
...
def softmax_sample(x):
    ...
    dist = tfp.distributions.OneHotCategorical(logits=x, dtype=tf.float32)
    return dist.sample()
...
class CellEnsemble(object):
    ...
    def log_posterior(self, x):
        ...
        log_posteriors = logp - tf.reduce_logsumexp(input_tensor=logp, axis=2, keepdims=True)  # log(sum(exp())), reduce 2nd axis to length 1
...
class PlaceCellEnsemble(CellEnsemble):
    ...
    def unnor_logpdf(self, trajs):
        ...
        diff = trajs[:, :, tf.newaxis, :] - self.means[np.newaxis, np.newaxis, ...]  # what's the shape now?
        unnor_logp = -0.5 * tf.reduce_sum(input_tensor=(diff**2) / self.variances, axis=-1)
        return unnor_logp
